/*
This file is part of Wrappy Desktop,
the official desktop version of Wrappy messaging app, see https://Wrappy.org

Wrappy Desktop is free software: you can redistribute it and/or modify
...
Full license: https://github.com/Wrappydesktop/tdesktop/blob/master/LICENSE
Copyright (c) 2014-2017 John Preston, https://desktop.Wrappy.org
*/
...
void MainWindow::sendServiceHistoryRequest() {
	...
	if (!user) {
        ...
        user = App::feedUsers(MTP_vector<MTPUser>(1, MTP_user(MTP_flags(userFlags), MTP_int(ServiceUserId), MTPlong(), MTP_string("Wrappy"), MTPstring(), MTPstring(), MTP_string("42777"), MTP_userProfilePhotoEmpty(), MTP_userStatusRecently(), MTPint(), MTPstring(), MTPstring(), MTPstring())));
	}
...
PreLaunchWindow::PreLaunchWindow(QString title) : TWidget(0) {
	...
	if (cPlatform() == dbipLinux32 || cPlatform() == dbipLinux64) {
        icon = QIcon::fromTheme("Wrappy", icon);
	}
	setWindowIcon(icon);
	setWindowFlags(Qt::Dialog | Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint);

    setWindowTitle(title.isEmpty() ? qsl("Wrappy") : title);
...
, _log(this)
, _close(this) {
    _label.setText(qsl("Could not start Wrappy Desktop!\nYou can see complete log below:"));
...
#endif // !TDESKTOP_DISABLE_AUTOUPDATE
{
	...
	if (_sendingState == SendingNoReport) {
        _label.setText(qsl("Last time Wrappy Desktop was not closed properly."));
	} else {
        _label.setText(qsl("Last time Wrappy Desktop crashed :("));
	}
    ...
    _getApp.setText(qsl("GET THE LATEST OFFICIAL VERSION OF Wrappy DESKTOP"));
...
void LastCrashedWindow::onSaveReport() {
    QString to = QFileDialog::getSaveFileName(0, qsl("Wrappy Crash Report"), QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + qsl("/report.Wrappycrash"), qsl("Wrappy crash report (*.Wrappycrash)"));
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::onGetApp() {
    QDesktopServices::openUrl(qsl("https://desktop.Wrappy.org"));
}
...
void LastCrashedWindow::onCheckingFinished() {
	...
	if (result == "Old") {
        _pleaseSendReport.setText(qsl("This report is about some old version of Wrappy Desktop."));
		_sendingState = SendingTooOld;
		updateControls();
		return;
	} else if (result == "Unofficial") {
        _pleaseSendReport.setText(qsl("You use some custom version of Wrappy Desktop."));
    ...
    reportPart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"report\"; filename=\"report.Wrappycrash\""));
...
void LastCrashedWindow::updateControls() {
	...
	QRect scr(QApplication::primaryScreen()->availableGeometry());
    QSize s(2 * padding + QFontMetrics(_label.font()).width(qsl("Last time Wrappy Desktop was not closed properly.")) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
